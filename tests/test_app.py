...
def test_update_user_not_valid(client, user, token):
    response = client.put(
        '/users/2',
        headers={'Authorization': f'Bearer {token}'},
        json={
...
def test_get_token_not_user(client, user, token):
    response = client.post(
        '/token',
        data={'username': 'invalid name', 'password': user.clean_password},
    )
...
def test_get_token_invalid_password(client, user):
    response = client.post(
        '/token', data={'username': user.email, 'password': 'invalid password'}
    )
